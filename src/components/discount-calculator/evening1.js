...
import { evening1 } from '../../constants/timePeriod';
...
class DiscountCalculatorEvening1 extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      currentIndex: 0,
    ...
    this.questions = buildOptions(evening1, props.data, isStudent);
    // should we move to other page?
    if (!props.data.timePeriodEvening.includes(evening1)) {
      props.history.push('/discount-calculator/evening2');
    }
  }
  ...
  onNextClick() {
    ...
    for (curr = this.state.currentIndex + 1; curr < this.questions.length; curr += 1) {
      ...
      if (this.state.answerMatrix[data.value] && this.state.answerMatrix[data.value].length === 0) {
        break;
      }
    }
    ...
    if (curr < this.questions.length) {
      ...
      this.props.addToData('answers_evening1', this.state.answerMatrix);
      this.props.history.push('/reason/evening1');
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(DiscountCalculatorEvening1);
